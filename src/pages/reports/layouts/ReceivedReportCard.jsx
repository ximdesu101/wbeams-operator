    ...
    return (
        <div className="rounded-xl border border-border bg-background p-4">
            ...
            {/* ── Staged action buttons ───────────────────────────────────
                pending      → Acknowledge only
                acknowledged → Reject + Resolve
                resolved / rejected → no buttons (card is in Recent tab)
            ────────────────────────────────────────────────────────────── */}
            {status === "pending" && (
                <CardFooter className="mt-4 flex flex-wrap gap-2 border-t-0 bg-transparent px-0 py-0">
                    <Button
                        ...
                        disabled={isUpdating}
                        onClick={() =>
                            onUpdateStatus?.({ id: report.id, status: "acknowledged" })
                        }
                    >
            ...
            {status === "acknowledged" && (
                <CardFooter className="mt-4 flex flex-wrap gap-2 border-t-0 bg-transparent px-0 py-0">
                    <Button
                        ...
                        disabled={isUpdating}
                        onClick={() =>
                            onUpdateStatus?.({ id: report.id, status: "resolved" })
                        }
                    >
                    ...
                    <Button
                        ...
                        disabled={isUpdating}
                        onClick={() =>
                            onUpdateStatus?.({ id: report.id, status: "rejected" })
                        }
                    >
